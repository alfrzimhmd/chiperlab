...
import { useGoogleTranslateGuard } from './hooks/useGoogleTranslateGuard';
...
import { EncryptPlayground } from './pages/playground/encrypt/EncryptPlayground';
import { HashPlayground } from './pages/playground/hash/HashPlayground';
import { AnalyzePlayground } from './pages/playground/analyze/AnalyzePlayground';
 ...
 * AppShell
 *
 * Wrapper component that lives INSIDE <HashRouter> so it can use router hooks.
 * This is where we call `useGoogleTranslateGuard` — the hook needs `useLocation`
 * and therefore must be rendered inside the Router context.
 */
function AppShell() {
  // Detects Google Translate and forces full reload on navigation when active.
  // Prevents the "blackscreen on SPA navigation while translated" bug.
